...
const EditBloodIssue = () => {
  ...
  const { isLoading: puloading, doPut } = usePutApi(`/api/issue/${id}`);
  ...
  const getData = async () => {
    try {
      ...
      if (data) {
        setdoi(
          `${data.doi.toString().slice(0, 4)}-${data.doi
            .toString()
            .slice(4, 6)}-${data.doi.toString().slice(6)}`
        );
  ...
  if (bloodcomponents && BloodComponent !== " ") {
    ...
    const findamount = bloodcomponents.find(
      (p) => p.id === Number(BloodComponent)
    );
    ...
    <>
      ...
      >
        ...
        <div className=" p-5 container" style={{minHeight:"80vh"}}>
          <div>
            <h1 className="Heading">Edit Blood Issue</h1>
          </div>
